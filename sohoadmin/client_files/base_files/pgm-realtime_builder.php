...
if($_GET['rmtemplate'] != '' && $_SESSION['PHP_AUTH_USER'] != '' && $_SESSION['PHP_AUTH_PW'] != ''){
	...
	if($custommenu == 'yes') {
		...
		if($seol->get("pref") == "yes") {
			$pageq = mysql_query('select page_name, link from site_pages order by page_name DESC');
   ...
   $template_footer = eregi_replace("#plugin-(.*)#", '', $template_footer);
	...
	if(eregi("name=\"required_fields\"", $template_footer) && eregi('action="pgm-form_submit\.php">', $template_footer)){
		...
		$jscript .= "}\n";
		$jscript .= "</script>\n";
		$template_footer = eregi_replace('action="pgm-form_submit\.php">', 'onSubmit="return camcheckrequired(this)" action="pgm-form_submit.php">', $jscript.$template_footer);
	}
