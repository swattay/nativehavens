            ...
            // Define the bullets
		      if ($tmp == $pageRequest || $tmp == $dup) {
				...
				if ($MENUTYPE == "buttons") {
					...
					$sub_buttons .= "<TR><TD class=$subClass width=100%><a href=\"index.php?pr=$tmp\"> >> $subpage_name[$xedusvar]</a></TD></TR>";
				}
				// ----------------------------------------------------------------------------------------
...
if ($MENUTYPE == "buttons") {
	...
	$main_buttons = "<TABLE BORDER=0 CELLPADDING=0 CELLSPACING=0 width=100%>\n";
}
...
while ($row = mysql_fetch_array ($result)) {
		...
		if ($MENUTYPE == "buttons") {
			...
			$main_buttons .= "<TR><TD CLASS=$mmClass width=100%><a href=\"index.php?pr=$pagelink\">$thispage</a></TD></TR>\n";
		}
		...
		if ($pagelink == $pageRequest || $pagelink == $dup) {
			...
			if ($numsubpages != 0 ) {
				...
				if ($MENUTYPE == "buttons") {
					...
					$main_buttons .= "<TR><TD ALIGN=LEFT width=100%>".$sub_buttons."</TD></TR>\n";
				}
